...
uint16_t send_packets(struct rte_mempool *mp, uint16_t port_id) {
	...
	for (uint8_t i = 0; i < BURST_SIZE; i++) {
		...
		struct ether_hdr *eth_hdr = (struct ether_hdr*)rte_pktmbuf_append(buf[i], sizeof(struct ether_hdr));
		...
		struct ipv6_hdr *ip_hdr = (struct ipv6_hdr*)rte_pktmbuf_append(buf[i], sizeof(struct ipv6_hdr));
		// Given dst_addr
		...
		ip_hdr->payload_len = rte_cpu_to_be_16(sizeof(struct tcp_hdr));

		// 3. Append L4 TCP SYN header and set it
		struct tcp_hdr *tcp_hdr = (struct tcp_hdr*)rte_pktmbuf_append(buf[i], sizeof(struct tcp_hdr));
